...
The raw axis and button order is the order reported by the WebXR runtime for that controller. In the current multi-camera XR renderer, the left-controller **X** button is read as `xrGamepad.buttons[4]` for the in-VR layout edit toggle. That means it appears in the published XR Joy message as `buttons[4]`, and its analog value appears at `axes[xrGamepad.axes.length + 4]`.
