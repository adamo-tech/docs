...
This is the sender side. Pair it with the robot-side receiver on the [Receiving Control](/control/#custom-control-topics) page.
...
```tsx
import { AdamoProvider, Stream, GamepadController, useOrg } from "@adamo/react";
...
function Teleop() {
  ...
  return (
    <>
      ...
      <GamepadController topic={`adamo/${org}/my-arm/control/joy`} />
    </>
  );
}

// The relay URL, org, and auth all resolve from the API key.
