---
title: "Rust SDK"
description: "Complete reference for the adamo crate — Session, pub/sub, queries, liveliness, latency, video, and fabric time"
---
...
```sh
cargo add adamo@^0.1.103
cargo add adamo@^0.1.103 --features video    # video capture + caller-fed tracks
cargo add adamo@^0.1.103 --features audio    # Opus microphone streaming (implies video)
```

Prebuilt `libadamo` is available for `aarch64-unknown-linux-gnu` (Jetson / Ubuntu 22.04 arm64 — bundled in the crate, offline-friendly), `x86_64-unknown-linux-gnu`, and `aarch64-apple-darwin` (fetched at build time, pinned by version and sha256; requires `curl`). All prebuilts include video. Other targets can point at a local build via `ADAMO_LIB_DIR=/path/to/dir/with/libadamo.{so,dylib}`.
...
Like `Session::open`, but additionally mints a per-session mTLS client certificate and presents it on the QUIC handshake — required once your routers enforce mTLS.
...
impl Default for PublishOptions {
    fn default() -> Self {
        Self { priority: Priority::DATA, express: false }  // DATA = 100
    }
}
```
...
impl Default for PublisherOptions {
    fn default() -> Self {
        Self { priority: Priority::DATA, express: false, reliable: false }
    }
}
```
...
A robot-owned retained key/value store — see [Robot State](/state/). Tied to its `Session` by lifetime; dropping it stops answering reads.
...
```rust
// subtasks: &[(id, name)] in order
pub fn select(&self, task_set_id: &str, name: &str, subtasks: &[(&str, &str)]) -> Result<()>;
pub fn advance(&self) -> Result<()>;   // one pedal press
pub fn reset(&self) -> Result<()>;     // back to idle without counting a rep
```
...
### `VideoBackend` and `VideoOptions`

```rust
pub enum VideoBackend {
    Auto,          // choose the default for this source and host
    GStreamer,     // force the GStreamer pipeline
    HwPipeline,    // force the native hardware pipeline
}

pub struct VideoOptions {
    pub width: u32,
    pub height: u32,
    pub pixel_format: Option<String>, // raw layout or h264/h265 input codec
    pub codec: String,                // output: h264, h265, or av1
    pub encoder: Option<String>,      // None = auto-detect
...
`VideoOptions::default()` produces a 1280×720, 30 fps H.264 track at
...
impl Robot {
    ...
    /// Existing iceoryx2 SHM service. The SDK owns decoding/encoding.
    pub fn attach_shm(
        &mut self,
        name: &str,
        service: &str,          // "camera/front"
        width: u32,
        height: u32,
        pixel_format: &str,     // "BGRA", "NV12", "mjpeg", ...
        fps: u32,
        bitrate_kbps: u32,
        stereo: bool,
    ) -> Result<()>;

    /// Existing SHM service with explicit input codec, output codec, encoder,
    /// backend, and passthrough control.
...
let mut robot = Robot::new(&api_key, Some("my-robot"), Protocol::Quic)?;
...
This decodes H.265 from shared memory and sends H.264 through Adamo. Use
...
See [Adding Cameras: transcoding encoded shared memory](/cameras/#transcode-h264-or-h265-from-shared-memory)
...
The `audio` feature adds microphone capture and Opus streaming to `Robot`. It
**implies `video`** (the `Robot` handle is shared), and needs an audio-enabled
`libadamo` — the published prebuilt carries it from `adamo` 0.1.98 onward.

```sh
cargo add adamo --features audio
```

Audio is **send-only** in the Rust SDK, matching video — the robot captures and
publishes; decode audio from the browser or the C/C++ SDK. See
[Streaming Audio](/audio/) for source types, defaults, and hardware notes.

### `AudioOptions`

```rust
pub struct AudioOptions {
    pub source_type: String,   // "auto" | "alsa" | "pipeline" | "test"
    pub device: Option<String>,   // ALSA device, e.g. "plughw:1,0"
    pub pipeline: Option<String>, // GStreamer source (source_type = "pipeline")
    pub codec: String,            // "opus"
    pub bitrate_kbps: u32,        // 64
    pub sample_rate: u32,         // 48_000
    pub channels: u8,             // 2 — ceiling, not a target
    pub frame_ms: u32,            // 20 — one of 2 (=2.5 ms), 5, 10, 20, 40, 60
    pub inband_fec: bool,         // false
    pub dtx: bool,                // false
    pub allow_missing: bool,      // false
}
```

`AudioOptions::default()` is Opus 64 kbps stereo, 48 kHz, 20 ms frames, FEC/DTX
off, `auto` source. Builder helpers: `with_source_type`, `with_device`,
`with_pipeline`, `with_bitrate_kbps`, `with_channels`, `with_sample_rate`,
`with_frame_ms`, `with_inband_fec`, `with_dtx`, `with_allow_missing`.

### `Robot` audio methods

```rust
impl Robot {
    /// Synthetic test tone — no hardware.
    pub fn attach_audio_test(&mut self, name: &str) -> Result<()>;

    /// Direct ALSA capture. `device` None uses the ALSA default.
    pub fn attach_audio_alsa(
        &mut self,
        name: &str,
        device: Option<&str>,   // "plughw:1,0"
        bitrate_kbps: u32,
        channels: u8,
        sample_rate: u32,
        frame_ms: u32,
    ...
    /// A GStreamer source string ending in raw audio.
...
```rust
use adamo::{AudioOptions, Robot};

let mut robot = Robot::new_default("ak_...", Some("my-robot"))?;

// Quick start: synthetic tone.
robot.attach_audio_test("mic")?;

// A real microphone with full options:
...
A failure to attach surfaces as `Error::Audio`.
...
pub enum Error {
    ...
    Video(String),
    /// Audio pipeline failure: capture device, encoder, or source setup failed.
    Audio(String),
    /// An uncategorized error reported by the underlying SDK library.
    Ffi(String),
...
Match on the variant to branch by failure category — no string parsing needed:

```rust
match Session::open_default(&api_key) {
    Ok(session) => { /* ... */ }
    Err(Error::Auth(msg)) => eprintln!("check the API key: {msg}"),
    Err(Error::Network(msg)) => eprintln!("connectivity problem: {msg}"),
    Err(e) => eprintln!("{e}"),
}
```

The enum is `#[non_exhaustive]` — always include a catch-all arm.
