...
### Troubleshooting high H.264 decode latency on Intel Linux publishers
...
```python
# Recommended fix while retaining GStreamer capture and preprocessing
robot.attach_video(
    "main",
...
This keeps GStreamer in charge of capture and preprocessing, then uses Adamo's
low-latency Intel hardware encoding before publishing the stream.
...
Intel VA-API encoder to avoid the one-frame decoder delay seen on affected
devices.
...
This does not add another process or require a shared-memory bridge. Adamo
validates that the pipeline output and host support the low-latency route. If
validation fails, startup reports the incompatible format or capability
instead of silently using a known high-latency configuration.
...
unchanged, so that stream does not gain Adamo's low-latency encoding behavior.
