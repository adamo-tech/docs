...
Attach a track driven by the SDK pipeline. Pass exactly one of `device=` (V4L2 device path or index), `shm=` (iceoryx2 service name produced by another process), `ros=` (ROS 2 topic — see [ROS 2](/ros/)), or `pipeline=` (legacy GStreamer string — deprecated).
