---
title: "Python SDK"
description: "Complete reference for the adamo Python package — connect, Robot, pub/sub, queries, liveliness, video, and recorded data"
---

import { Aside } from "@astrojs/starlight/components";

The `adamo` Python package is the full Adamo SDK — pub/sub, queries, liveliness, hardware-encoded video, Opus [audio](/audio/), and recorded-data access for training. Python 3.10+.

## Install

```sh
pip install adamo               # core: pub/sub, queries, liveliness
pip install 'adamo[video]'      # adds iceoryx2 + numpy for video tracks
pip install 'adamo[data]'       # adds pandas for DataFrame loading
pip install 'adamo[ml]'         # adds torch + av for PyTorch datasets
```

The package builds from a Rust extension via maturin. Pre-built wheels are published for the supported platforms; on others, `pip` will compile from source (requires Rust toolchain).

## Top-Level API

```python
import adamo
```

Re-exports:

| Symbol | From | Description |
|--------|------|-------------|
| `connect` | `adamo.operate._config` | Open a `Session` from an API key. |
| `connect_async` | `adamo.operate._config` | Async variant of `connect`. |
| `Session` | `adamo.operate.session` | The pub/sub / query / liveliness session. |
| `Sample` | `adamo.operate.session` | A received message. |
| `Publisher` | `adamo.operate.session` | A persistent publisher. |
| `Subscriber` | `adamo.operate.session` | An iterator over received samples. |
| `Robot` | `adamo.session` | A participant — combines pub/sub with hardware video. |
| `Participant` | `adamo.session` | Alias for `Robot`. |
| `VideoTrack` | `adamo._video` | Python-fed video track (lazy import). |
| `data` | `adamo.data` | Recorded-session client (lazy import). |
| `operate` | `adamo.operate` | The pub/sub submodule (lazy import). |

`adamo.JointState`, `adamo.Joy`, `adamo.JoystickCommand` are exposed via `from adamo import …` (re-exports of `adamo.operate.control`).

---

## `connect`

```python
adamo.connect(
    *,
    api_key: str | None = None,
    token: str | None = None,
    org_id: str | None = None,
    api_url: str = "https://api.adamohq.com",
    protocol: str = "quic",
    mtls: bool = False,
) -> Session
```

Open an authenticated session. Provide an `api_key` for live SDK sessions.
Token auth is still used by browser/user flows, but token-based Python live sessions are not wired in the native transport yet.

| Arg | Description |
|-----|-------------|
| `api_key` | An API key starting with `ak_`. Used for robots and scripts. |
| `token` | A Supabase JWT. Used for user-authenticated sessions. |
| `org_id` | Required only with `token` if the user belongs to multiple orgs. |
| `api_url` | Override the Adamo API base URL. |
| `protocol` | Transport — `"quic"` (default; use it in all cases), `"udp"`, or `"tcp"` (telemetry/diagnostic fallback — don't teleoperate over TCP). |
| `mtls` | Use the mTLS QUIC path when routers require client certificates. |

Raises `ValueError` if both `api_key` and `token` are passed, or if neither is. Raises `NotImplementedError` for token-based native sessions.

```python
adamo.connect_async(*, api_key=..., token=..., org_id=..., api_url=..., protocol="quic", mtls=False) -> Session
```

Async variant — does the API config fetch asynchronously, then opens a synchronous session. Returns a `Session`, not a coroutine of one (the underlying transport is sync).

---

## `Session`

```python
class adamo.operate.Session
```

Every key you pass is automatically scoped to your organisation; you write `{robot}/{topic}` (e.g. `my-arm/sensors/imu`).

### `Session.org`

```python
@property
session.org -> str
```

The organisation slug resolved from the API key.

### `Session.put`

```python
session.put(
    key_expr: str,
    payload: bytes | str,
    *,
    raw: bool = False,
    priority: Priority = Priority.DATA,
    express: bool = False,
    reliable: bool = False,
) -> None
```

One-shot publish. `payload` is encoded as UTF-8 if a `str` is passed. `raw=True` skips the org-scope prefix (useful for router-side endpoints).

### `Session.publisher`

```python
session.publisher(
    key_expr: str,
    *,
    raw: bool = False,
    priority: Priority = Priority.DATA,
    express: bool = False,
    reliable: bool = False,
) -> Publisher
```

Declare a persistent `Publisher` for repeated puts on the same key. The publisher is undeclared on `.close()` or context exit.

### `Session.subscribe`

```python
session.subscribe(
    key_expr: str,
    *,
    raw: bool = False,
    callback: Callable[[Sample], None] | None = None,
) -> Subscriber
```

Declare a subscriber. Without `callback`, the returned `Subscriber` is iterable and supports non-blocking `try_recv()`. With `callback`, samples are delivered to it on the receive thread; the returned handle is used for lifecycle (`close()`).

Wildcards in `key_expr`:

| Pattern | Matches |
|---------|---------|
| `my-arm/sensors/**` | Everything under `sensors/`. |
| `my-arm/sensors/*`  | One level under `sensors/`. |
| `*/sensors/imu`     | IMU from any robot. |

### `Session.get`

```python
session.get(
    key_expr: str,
    *,
    raw: bool = False,
    timeout_ms: int = 5000,
) -> list[Sample]
```

One-shot query. Collects every reply that arrives within `timeout_ms`. Returns an empty list if nothing replies.

### `Session.alive`

```python
session.alive(token_key: str) -> LivelinessToken
```

Declare this client as alive at `{token_key}/alive`. The token stays active until you call `.close()` on it (or the process exits).

### `Session.live_tokens`

```python
session.live_tokens(pattern: str = "**/alive") -> list[str]
```

Query currently-live tokens matching `pattern`. Returns prefix-stripped keys. Default returns every live token in the org.

### `Session.on_liveliness`

```python
session.on_liveliness(
    pattern: str = "**/alive",
    *,
    callback: Callable[[str, bool], None] | None = None,
    history: bool = True,
) -> Subscriber
```

Watch for liveliness changes. The callback fires with `(key, is_alive)` whenever a token appears or disappears. `history=True` delivers the current set of live tokens up front so the handler sees them on subscribe.

### `Session.state_store`

```python
session.state_store(robot: str) -> StateStore
```

Declare a robot-owned [key/value store](/state/) on `{robot}/state/**`. The robot updates it; operators read with `get` / `subscribe`.

### `Session.task_runner`

```python
session.task_runner(robot: str) -> TaskRunner
```

Declare a [task runner](/state/#task-runs) — a looping subtask sequence with an idle gap between reps — published on `{robot}/state/task_run`.

### `Session.fabric_now_us`

```python
session.fabric_now_us() -> int
```

Microseconds since the Unix epoch on the Adamo fabric clock. Until the first time sync completes, this falls back to the local wall clock.

### `Session.fabric_synced`

```python
session.fabric_synced() -> bool
```

`True` once the first fabric time sync has completed (typically under 100 ms after connect).

### `Session.close`

```python
session.close() -> None
```

Close the underlying connection. Also reachable via `with adamo.connect(…) as session:`.

---

## `Sample`

```python
class adamo.Sample
```

A received message.

| Attribute | Type | Description |
|-----------|------|-------------|
| `key` | `str` | Topic key with the org scope stripped. |
| `payload` | `bytes` | Raw payload. |
| `timestamp` | `Timestamp \| None` | Sender timestamp, if the publisher set one. |

`__repr__` shows key + payload size.

---

## `Publisher`

```python
class adamo.Publisher
```

A persistent publisher tied to a single key.

```python
publisher.put(payload: bytes | str) -> None
publisher.close() -> None
```

`str` payloads are encoded as UTF-8. Supports the context-manager protocol — exit closes (undeclares) the publisher.

---

## `Subscriber`

```python
class adamo.Subscriber
```

An iterator over received samples.

```python
for sample in subscriber:
    ...

sample = subscriber.try_recv()  # Sample | None — non-blocking
subscriber.close()              # undeclares the subscription
```

Supports the context-manager protocol.

---

## `StateStore`

```python
class adamo.StateStore
```

A robot-owned retained key/value store — see [Robot State](/state/).

```python
store.set(key: str, value) -> None    # dict/list/str -> JSON; bytes sent as-is
store.get(key: str) -> bytes | None
store.get_json(key: str)              # parsed JSON, or None
store.delete(key: str) -> None
```

---

## `TaskRunner`

```python
class adamo.TaskRunner
```

A looping subtask sequence with an idle gap between reps; publishes `{robot}/state/task_run`. See [Task Runs](/state/#task-runs).

```python
# subtasks: list of {"id", "name"} dicts or (id, name) tuples
runner.select(task_set_id: str, name: str, subtasks) -> None
runner.advance() -> None    # one pedal press
runner.reset() -> None      # back to idle without counting a rep
```

---

## `Robot`

```python
class adamo.Robot:
    def __init__(
        self,
        api_key: str | None = None,
        name: str | None = None,
        relay: str | None = None,
        protocol: str = "quic",
    ): ...
```

A participant — combines pub/sub messaging with hardware-encoded video. The same class covers three roles, defined by which methods you call:

- **Leader / viewer** (no camera): use `publish` / `subscribe` / `send` / `recv`.
- **Follower robot** (one or more cameras): use `attach_video` for each camera + `subscribe` for control.
- **Perception pipeline** (Python frame loop): use `video()` to get a `VideoTrack`, then `track.send(frame)` in a loop.

`Participant` is exposed as an alias of `Robot` for the leader / viewer case.

### Properties

```python
robot.session -> Session   # opens the underlying Session lazily
```

### Video attachments

```python
robot.attach_video(
    name: str,
    *,
    device: str | int | None = None,
    shm: str | None = None,
    pipeline: str | None = None,         # legacy GStreamer path; deprecated
    ros: str | None = None,              # ROS 2 topic (see the ROS 2 guide)
    ros_format: str | None = None,       # override inferred handling, e.g. "ros_h264"
    codec: str = "h264",
    encoder: str | None = None,
    bitrate_kbps: int = 2000,
    adaptive_bitrate: bool = True,
    min_bitrate_kbps: int | None = None,
    max_bitrate_kbps: int | None = None,
    bitrate_priority: float = 1.0,
    fps: int = 30,
    width: int = 1280,
    height: int = 720,
    pixel_format: str | None = None,
    backend: str | None = None,          # "gstreamer" or "hw_pipeline"
    keyframe_distance: float = 2.0,
    stereo: bool = False,
    reliability: str | None = None,
    shm_publish: str | None = None,
    passthrough: bool = False,
    allow_missing: bool = False,
) -> None
```

Attach a track driven by the SDK pipeline. Pass exactly one of `device=` (V4L2 device path or index), `shm=` (iceoryx2 service name produced by another process), `ros=` (ROS 2 topic — see [ROS 2](/ros/)), or `pipeline=` (legacy GStreamer string — deprecated). See [GStreamer pipelines](/cameras/#gstreamer-pipelines) for backend selection and encoded-pipeline passthrough.

```python
robot.enable_ros_control(
    *,
    domain_id: int = 0,                       # 0 honors ROS_DOMAIN_ID
    control_channels: list[str] | None = None,
    control_topics: list[str] | None = None,  # allowlist; None = unrestricted
) -> None
```

Fan operator control out to the local ROS 2 graph as native topics — see [ROS 2](/ros/#operator-control-as-ros-topics). Requires a ROS-enabled build.

```python
robot.video(
    name: str,
    *,
    width: int = 1280,
    height: int = 720,
    pixel_format: str,            # required — raw buffers carry no layout
    codec: str = "h264",
    encoder: str | None = None,
    bitrate_kbps: int = 2000,
    adaptive_bitrate: bool = True,
    min_bitrate_kbps: int | None = None,
    max_bitrate_kbps: int | None = None,
    bitrate_priority: float = 1.0,
    fps: int = 30,
    backend: str | None = None,   # "gstreamer" or "hw_pipeline"
    keyframe_distance: float = 2.0,
    stereo: bool = False,
) -> VideoTrack
```

Create a track whose frames you push from Python. Returns a `VideoTrack`; call `.send(frame)` with a numpy array (or bytes-like). The encoder pipeline auto-starts on the first `send()` if you haven't called `robot.run()` yourself. `pixel_format` is required and raises `ValueError` when omitted. With `adaptive_bitrate` (the default) the encoder follows the congestion forecast between `min_bitrate_kbps` and `max_bitrate_kbps`; `bitrate_priority` weights this track's share when several tracks compete.

### Audio attachments

```python
robot.attach_audio(
    name: str,
    *,
    source_type: str = "auto",   # "auto" | "alsa" | "pipeline" | "test"
    device: str | None = None,   # ALSA device, e.g. "plughw:1,0"
    pipeline: str | None = None, # GStreamer source string (source_type="pipeline")
    codec: str = "opus",
    bitrate_kbps: int = 64,
    sample_rate: int = 48000,
    channels: int = 2,           # ceiling, not a target — a mono source stays mono
    frame_ms: int = 20,          # 2 (=2.5 ms), 5, 10, 20, 40, 60
    inband_fec: bool = False,
    dtx: bool = False,
    allow_missing: bool = False, # keep the robot running if the source is absent
) -> None
```

Attach a microphone track, captured and Opus-encoded by the SDK and published on
`{robot.name}/audio/{name}`. Audio is **send-only** in the Python SDK (consume it
from the browser or the C/C++ SDK, matching video). See [Streaming Audio](/audio/)
for source types and hardware notes.

`source_type="pipeline"` requires `pipeline=` and raises `ValueError` otherwise.
An audio-only robot (no video tracks) still drives the capture pipeline from
`run()`. Requires `adamo` 0.4.48+; the Linux x86_64 wheel supports
`source_type="alsa"` only, while the aarch64 wheel supports every source type.

### Pub/sub

```python
robot.publish(
    track: str,
    *,
    priority: int = 200,    # 0–255; ≥240 maps to REAL_TIME
    express: bool = False,
    reliable: bool = False,
) -> Publisher
```

Declare a publisher on `{robot.name}/{track}`. The numeric `priority` is mapped to one of 8 transport priority classes (see [`_to_zenoh_priority`](#priority-mapping)).

```python
robot.subscribe(
    broadcast: str,
    track: str | list[str],
    callback: Callable[..., None],
    *,
    priority: int = 200,
) -> Callable
```

Subscribe to one or more tracks on another participant's broadcast. `broadcast` is the participant name (e.g. `"xr-operator"`); `track` can include `*` / `**` wildcards or `{name}` capture groups. Named captures are passed to the callback as keyword arguments matching the parameter names.

Returns the callback unchanged so it works as a decorator too.

```python
@robot.on(
    broadcast: str,
    track: str | list[str],
    *,
    priority: int = 200,
    decode: Callable[[bytes], object] | str | None = "json",
) -> Callable
```

Decorator. `decode` controls payload transformation before the handler runs:

- `"json"` (default) — `json.loads(payload)`.
- `"control"` — `decode_control(payload)` (returns `JointState` / `Joy` / `JoystickCommand` / dict).
- `None` — pass the raw `bytes`.
- Any callable — called with raw bytes; its return value is passed in.

`"control"` is for the JSON control models below. For the browser's ROS
envelope/CDR Joy and XR streams, use the typed `adamo.xr` API.

### Messaging (back-channel)

```python
robot.send(channel: str, data: bytes | str) -> None
```

Publish a message on `{robot.name}/msg/{channel}`. `str` data is encoded as UTF-8.

```python
robot.recv(timeout: float | None = None) -> tuple[str, bytes]
```

Block until a message arrives on any `msg/*` channel from another participant. Returns `(channel, data)`. Raises `TimeoutError` if `timeout` elapses.

```python
robot.on_message(callback: Callable[[str, bytes], None]) -> Callable
```

Register a callback for incoming messages. Can be used as a decorator. Multiple callbacks can be registered.

### Logging

```python
robot.log(message: str, *, level: str = "info") -> None
```

Publish a log line from this robot. The web operator console subscribes to this stream and renders the log lines in real time.

`level` is a free-form string; the standard values `"info"`, `"warn"`, `"error"`, `"debug"` are rendered with colour by the built-in UI. Messages are truncated at 10,000 characters.

```python
import adamo

robot = adamo.Robot(api_key="ak_...", name="my-robot")
robot.log("booted and attached camera")
robot.log("encoder dropped a frame", level="warn")
try:
    do_stuff()
except Exception as e:
    robot.log(f"pipeline failed: {e}", level="error")
```

Each log entry is stamped with the fabric clock so lines from multiple robots stay ordered in the operator view.

### Lifecycle

```python
robot.run() -> None
```

Block until the session ends. If video tracks are attached, drives the encoder pipeline. Otherwise sleeps until Ctrl+C.

```python
robot.close() -> None
```

Tear down all subscribers, publishers, video tracks, and the session. Supports the context-manager protocol.

---

## `VideoTrack`

```python
from adamo import VideoTrack
```

Returned from `robot.video()`. Push frames to feed the encoder.

| Attribute / method | Type | Description |
|--------------------|------|-------------|
| `name` | `str` | Track name. |
| `width`, `height` | `int` | Frame dimensions. |
| `pixel_format` | `str` | Uppercased format string. |
| `service_name` | `str` | iceoryx2 service backing the track. |
| `send(frame)` | `None` | Push one frame (numpy array or bytes-like). Length must match `width × height × bytes-per-pixel` for the format. |
| `close()` | `None` | Mark the track closed; subsequent `send()` raises `RuntimeError`. |

Supported `pixel_format` values:

| Format | Bytes per pixel |
|--------|------------------|
| `BGRA`, `RGBA`, `BGRX`, `RGBX` | 4 |
| `RGB`, `BGR` | 3 |
| `YUY2`, `UYVY` | 2 |
| `I420`, `NV12` | 1.5 |

Non-contiguous numpy arrays are copied to contiguous before sending.

---

## Control message types

```python
from adamo.operate.control import JointState, Joy, JoystickCommand, decode_control
```

JSON-serialisable dataclasses for the most common control payloads. Each has a `.to_json() -> bytes` method that fills `stamp` with `time.time()` if it's zero.

```python
@dataclass
class JointState:
    names: list[str]      = []
    positions: list[float] = []
    velocity: list[float]  = []
    effort: list[float]    = []
    stamp: float = 0.0
    frame_id: str = ""
```

```python
@dataclass
class Joy:
    axes: list[float] = []
    buttons: list[int] = []
    stamp: float = 0.0
```

```python
@dataclass
class JoystickCommand:
    sequence_id: int = 0
    axes: list[float] = []
    buttons: list[int] = []
    stamp: float = 0.0
```

```python
decode_control(payload: bytes) -> JointState | Joy | JoystickCommand | dict
```

Decodes a JSON payload by inspecting its `type` field. Unknown types come back as a raw `dict`.

## Typed ROS / XR control

Available in `adamo>=0.4.54`.

```python
from adamo.xr import (
    PoseStamped,
    XRControlSample,
    XRDecodeError,
    XRJoy,
    decode_ros_envelope,
    decode_xr_control,
    subscribe_xr_control,
)
```

The browser gamepad and XR streams use an Adamo ROS envelope containing CDR,
not the JSON format handled by `decode_control`. `adamo.xr` validates and
decodes the envelope plus `geometry_msgs/msg/PoseStamped` and
`sensor_msgs/msg/Joy` without requiring ROS or NumPy.

```python
sample = decode_xr_control(payload)

sample.topic                 # e.g. "/controller/left"
sample.type_name             # "geometry_msgs/msg/PoseStamped"
sample.source_timestamp      # ROS Header time in fabric/epoch seconds
sample.transport_timestamp   # Zenoh metadata time, when supplied
sample.is_stale(0.5)         # missing timestamps are stale

if isinstance(sample.message, PoseStamped):
    sample.message.header.frame_id
    sample.message.position.as_tuple()       # (x, y, z)
    sample.message.orientation.as_tuple()    # (x, y, z, w)
elif isinstance(sample.message, XRJoy):
    sample.message.axes
    sample.message.buttons
```

`RosTimestamp` retains integer `sec` and `nanosec` fields, so the exact source
timestamp is not lost. `Quaternion` always uses the ROS/WebXR scalar-last
`x, y, z, w` convention. The decoder accepts little- and big-endian CDR and
raises `XRDecodeError` for malformed envelopes, invalid alignment/bounds,
invalid strings, oversized sequences, or invalid ROS nanoseconds.

```python
subscribe_xr_control(
    session,
    robot_name: str,
    callback: Callable[[XRControlSample], None],
    *,
    channel: str = "cdr/xr_tracking",
    max_age_seconds: float | None = None,
    on_decode_error: Callable[[XRDecodeError], None] | None = None,
)
```

This wraps `Session.subscribe` and returns the same lifecycle handle. When
`max_age_seconds` is set, messages with an unset or old ROS source timestamp
are dropped before the callback. Use `channel="joy"` for the browser gamepad
key; the default subscribes to XR tracking.

`decode_ros_envelope(payload)` is the lower-level validated helper for callers
that need the untouched CDR bytes, such as a bridge passing them to ROS
`deserialize_message`. `PoseArray` hand-joint messages are not yet decoded by
`decode_xr_control`; use the envelope helper plus ROS or a general CDR decoder.

---

## Priority mapping

`Robot.publish(priority=…)` accepts a 0–255 integer that's mapped to the 8-class priority enum:

| Range | Class |
|-------|-------|
| 240–255 | `REAL_TIME` |
| 210–239 | `INTERACTIVE_HIGH` |
| 180–209 | `INTERACTIVE_LOW` |
| 140–179 | `DATA_HIGH` |
| 100–139 | `DATA` |
| 60–99   | `DATA_LOW` |
| 0–59    | `BACKGROUND` |

Higher numeric values drain ahead of lower ones under congestion.

---

## Downloading Data

The `adamo.data` submodule connects to the recorded-data store rather than the live network. Use it to download recorded sessions, browse topics, decode video, build temporally-aligned tables, and produce PyTorch datasets for training.

This module is **Python only** — there is no equivalent in the Rust or C SDKs.

```python
from adamo.data import connect, DataClient, AdamoDataset, SessionMetadata, Record, VideoIndex, Frame
```

### `connect`

```python
adamo.data.connect(
    *,
    api_key: str,
    api_url: str = "...",
    store_url: str = "https://store.adamohq.com",
) -> DataClient
```

Open a `DataClient`. Exchanges the API key for a short-lived JWT internally and refreshes when it expires.

### `DataClient`

```python
class adamo.data.DataClient
```

#### Sessions

```python
client.list_sessions(
    *,
    after: str | float | datetime | None = None,
    before: str | float | datetime | None = None,
    name_contains: str | None = None,
) -> list[SessionMetadata]

client.get_session(session_id: str) -> SessionMetadata
client.get_topics(session_id: str) -> list[str]
client.match_topics(session_id: str, pattern: str) -> list[str]
client.message_count(session_id: str) -> int
```

`pattern` in `match_topics` supports `*` (single segment) and `**` (any depth).

#### Records

```python
client.query_records(
    session_id: str,
    *topics: str,
    start: str | float | datetime | None = None,
    end:   str | float | datetime | None = None,
) -> list[Record]

client.iter_records(...) -> Iterator[Record]    # streaming
client.export_records(..., chunk_size: int = 1000) -> Iterator[list[Record]]
```

`*topics` supports wildcards. Without arguments, returns every topic in the session.

#### Aligned queries

```python
client.aligned(
    session_id: str,
    *topics: str,
    start: ... = None,
    end: ... = None,
    window_ms: int = 50,
    hz: float | None = None,
) -> list[dict[str, Record]]
```

Temporally aligns records across two or more topic patterns. Returns one dict per timestep, mapping topic name to the matched `Record`. If `hz` is set, resamples to that frequency; otherwise aligns to the first topic's timestamps.

#### Video

```python
client.video_index(session_id: str, topic: str) -> VideoIndex
client.download_video(session_id: str, topic: str, output_path: str | Path) -> Path
client.iter_frames(
    session_id: str,
    topic: str,
    *,
    start: ... = None,
    end:   ... = None,
    size:  tuple[int, int] | None = None,
) -> Iterator[Frame]
```

`iter_frames` requires `pip install av numpy`. Each `Frame.image` is a `(H, W, 3)` uint8 RGB numpy array; `size=(w, h)` resizes.

#### Episodes

```python
client.episodes(
    *topics: str,
    sessions: list[str] | None = None,
    window_ms: int = 50,
) -> Iterator[dict[str, list[Record]]]
```

Iterate sessions as trajectory-level episodes. Each yielded dict maps topic name to a time-ordered list of `Record`s.

#### DataFrame

```python
client.to_dataframe(
    session_id: str,
    *topics: str,
    start: ... = None,
    end: ... = None,
)
```

Returns a pandas `DataFrame` with columns `session_id`, `topic`, `timestamp`, `payload`. Requires `pip install adamo[data]`.

#### Dataset

```python
client.dataset(
    sessions: list,
    observation: dict[str, str | tuple[str, str]],
    action: str | tuple[str, str],
    obs_steps: int = 1,
    action_steps: int = 1,
    hz: float = 30.0,
    image_size: tuple[int, int] | None = None,
) -> AdamoDataset
```

Build a PyTorch-compatible dataset. `observation` maps user keys to topic specs:

- `"topic/path"` — auto-detects video vs. raw bytes.
- `("topic/pattern", "field_name")` — JSON-decode payload, extract that field as a float array.

`action` takes the same form. Wildcards resolve automatically; if a pattern matches multiple topics the first is used and a warning is emitted.

#### Lifecycle

```python
client.close() -> None
```

Supports the context-manager protocol.

### `AdamoDataset`

```python
class adamo.data.AdamoDataset
```

A PyTorch-compatible dataset. All download / decode happens at construction; `__getitem__` is O(1).

```python
dataset[i] -> dict[str, torch.Tensor]
```

Each sample has dot-separated keys:

- `"observation.<key>"` for each entry in `observation`, shape `(obs_steps, …)`.
- `"action"`, shape `(action_steps, …)`.

Video observations are float32 in `[0, 1]` with shape `(obs_steps, 3, H, W)`. Numeric observations are `(obs_steps, D)`.

```python
dataset.stats: dict[str, dict[str, np.ndarray]]
# dataset.stats["observation.state"] = {"mean": …, "std": …}
```

Per-key mean and std for all non-video keys (plus the action key).

### Data models

```python
@dataclass
class SessionMetadata:
    id: str
    name: str
    status: str
    topics: list[str]
    started_at: float | None  # epoch seconds
    stopped_at: float | None
    message_count: int
    org_id: str
```

```python
@dataclass
class Record:
    session_id: str
    topic: str
    payload: bytes
    timestamp: float          # epoch seconds
```

```python
@dataclass
class VideoIndex:
    session_id: str
    topic: str
    frame_count: int
    keyframe_count: int
    duration_ms: int
    avg_fps: float
    segments: list[dict]
```

```python
@dataclass
class Frame:
    topic: str
    timestamp: float
    image: object             # (H, W, 3) uint8 RGB numpy array

    @property
    def height(self) -> int
    @property
    def width(self) -> int
```

---

## Concurrency model

- **Session methods are synchronous.** `connect_async` is the only async surface and only the API config fetch is async — the returned `Session` is the same sync object as `connect`.
- **Subscribe callbacks run on a background receive thread.** Keep them short. For heavy work, push to a queue and process from your main thread.
- **The video pipeline runs in its own threads.** `Robot.run()` blocks the calling thread driving it; `track.send()` is thread-safe.
- **`Robot.recv()` blocks** the calling thread until a `msg/*` message arrives.

## Errors

SDK-specific failures derive from `adamo.AdamoError`, so one handler catches everything Adamo-related while unrelated bugs surface normally:

| Exception | Raised when |
|---|---|
| `adamo.AuthError` | The Adamo API rejected the API key or token. |
| `adamo.NetworkError` | The Adamo API or a relay could not be reached. Also subclasses builtin `ConnectionError`, so existing generic handlers keep working. |

```python
try:
    session = adamo.connect(api_key="ak_...")
except adamo.AuthError:
    ...  # bad or inactive key — check operate.adamohq.com → Settings → API Keys
except adamo.AdamoError:
    ...  # any other SDK failure
```

The SDK also raises `ValueError` for bad arguments (mutually exclusive options, malformed specs), `RuntimeError` for state errors (closed track), `TimeoutError` from `Robot.recv(timeout=…)` and `measure_rtt`, and `ImportError` from optional features when their extras aren't installed.

## Doctor

`python -m adamo doctor` diagnoses the local setup: API key validity, QUIC reachability (including whether the network blocks UDP), encoder availability, glibc version, and the video extras. Run it first when anything misbehaves — the output is designed to be copy-pasted into a support request.

```text
$ python -m adamo doctor --api-key ak_...   # or set ADAMO_API_KEY
adamo doctor

  ✓ python 3.11.9
  ✓ adamo 0.4.32
  ✓ linux aarch64
  ✓ glibc 2.35
  ✓ API key valid (org: my-org)
  ✓ QUIC transport connected, relay RTT 12.4ms
  ✓ H.264 encoder: nvv4l2h264enc
  ✓ iceoryx2 0.8.1 (shared-memory video ready)
  ✓ numpy available

No problems found.
```
